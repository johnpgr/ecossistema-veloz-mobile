...
const serviceCategories = [
  {
    id: 1,
    name: "Ajuste de notas",
    description:
      "Solicitação de correção/atualização de notas lançadas no sistema.",
  },
  {
    id: 2,
    name: "Ass. termo de compromisso de estágio",
    description:
      "Assinatura e validação do termo de compromisso e aditivos de estágio.",
  },
  {
    id: 3,
    name: "Dispensa de disciplinas",
    description:
      "Pedido de dispensa por equivalência ou aproveitamento de estudos.",
  },
  {
    id: 4,
    name: "Inclusão de disciplinas",
    description:
      "Solicitação para adicionar disciplinas à sua matrícula atual.",
  },
  {
    id: 5,
    name: "Mudança de curso",
    description: "Solicitação de transferência interna para outro curso.",
  },
  {
    id: 6,
    name: "Mudança de turma",
    description: "Troca de turma/horário na mesma disciplina.",
  },
  {
    id: 7,
    name: "Mudança de turno",
    description: "Alteração de turno (manhã/tarde/noite) no curso.",
  },
  {
    id: 8,
    name: "Outros",
    description: "Demandas administrativas ou acadêmicas não listadas acima.",
  },
]
...
export default function AtendimentoFormScreen() {
  ...
  function handleSubmitRequest() {
    if (!selectedService || !requestDescription.trim() || !subject.trim()) {
      Alert.alert(
        "Error",
        "Please select a service type and provide a description and subject.",
      )
      return
    }
    ...
    <SafeAreaView className="flex-1 bg-gray-50">
      <View className="px-4 py-6 bg-green-800">
        ...
        <Text className="text-green-100 mt-1">
          Preencha o formulário com o conteúdo de seu atendimento
        </Text>
      </View>
      <KeyboardAvoidingView
        className="flex-1"
      ...
      >
        ...
        >
          <View className="bg-white rounded-lg shadow-sm p-4 mb-4">
            ...
            <View className="gap-1">
              <Text className="text-gray-700">
                <Text className="font-semibold">Matrícula: </Text>
                {currentStudent.enroll}
              </Text>
              <Text className="text-gray-700">
              ...
              <Text className="text-gray-700">
                <Text className="font-semibold">Curso: </Text>
                {currentStudent.course}
              </Text>
              <Text className="text-gray-700">
                <Text className="font-semibold">Período: </Text>
                {currentStudent.period_absolute ?? currentStudent.period}
              </Text>
              <Text className="text-gray-700">
                <Text className="font-semibold">Coordenador: </Text>
                {currentStudent.course_chief}
              </Text>
            </View>
          </View>
          <View className="bg-white rounded-lg shadow-sm p-4 mb-4">
            ...
            {serviceCategories.map((service) => (
              ...
              >
                <Text
                  className={`font-medium ${selectedService?.id === service.id ? "text-green-700" : "text-gray-800"}`}
                >
                  {service.name}
                </Text>
          ...
          <View
            ...
            style={{ marginBottom: Platform.select({ ios: 20, android: 30 }) }}
          >
            <TouchableOpacity
